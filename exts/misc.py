...
class Miscellaneous(commands.Cog):
    def __init__(self, bot):
        ...
        self.consoles = {
            ...
            'ds': { # melonds
                "links": "[Emulator](<https://melonds.kuribo64.net/downloads.php>), [ROMs](<https://vimm.net/vault/DS>), [BIOS](<https://mega.nz/file/KPwUHYCZ#mCzMRg3UN8UGJ2WKxAbCMaWVLUdAX0KCYHb0egCbrUk>)",
                "instructions": "1. Download emulator for your platform.\n2. Download your ROM of choice from the vault. Extract the ROM from the archive to a location of your choice.\n3. Download the BIOS files and extract the arcive to a location of your choice. Remember this location for later.\n4. Near where you left your ROM, create `saves`, `states`, and `cheats` folders for reference later.\n5. Launch melonDS. Click `Config > Emu settings`. Choose the `Console type` as DS, and make sure `Boot game directly` is UNTICKED!\n6. Click the `DS-mode` tab. Make sure `Use external BIOS/firmware files` is ticked.\n7. For each BIOS option you see on the window, click `Browse...`. From the folder you extracted eariler, match each `.bin` with the respective BIOS. Click `OK` at the bottom.\n8. Click `Config > Path settings`. Match each path you see on screen with the folders you created earlier. Click OK at the bottom.\n9. Click `Config > Input settings`. Edit the controls as you see fit. Optionally, click on the `General hotkeys` tab and adjust those. Click OK at the bottom.\n10. Click `File > Open ROM`. Locate where you stored your ROM and hit Open.\n\\- melonDS supports multiplayer through creating a second instance! Simply click `System > Multiplayer > Launch new instance`, then load your ROM by clicking on the new melonDS window and loading your ROM through `File > Open ROM`.\n\\- If that's not your thing, melonDS also supports Wi-Fi! In your game, enter the Wi-Fi settings, edit the DNS settings for `melonAP` by selecting `No` on `Auto-Obtain DNS`. Enter the primary DNS as `167.86.108.126` and the secondary as `1.1.1.1`.\n\\- From now on, your last 10 played DS ROMs are listed in `File > Open recent > (your ROM)`. Just click on your ROM from there.\n\\- To load more ROMs, simply click `File > Open ROM`."
            }, 
            '3ds': { # lime3ds / citra
