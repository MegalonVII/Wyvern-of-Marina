...
class Fun(commands.Cog):
    def __init__(self, bot):
        ...
        self.consoles = {
            ...
            'switch': { # ryubing
                "links": ["[Ryubing](<https://ryujinx.app/>)", "[ROMs](<https://sor.bz/6gr2P>)", "[Firmware](<https://prodkeys.net/ryujinx-firmware/>)", "[Keys](<https://prodkeys.net/ryujinx-prod-keys/>)", "[Ad blocker](<https://ublockorigin.com/>)"],
                "instructions": "1. Download the latest release of the emulator for your platform.\n2. Download your ROM of choice and any update and/or DLC from the vault by scrolling down to the downloads section. Use an adblocker when accessing the vault. Extract anything to a preferred location.\n3. Download the latest key files from the keys site. Extract the `.zip` file but do NOT delete it!\n4. Follow [this guide](<https://youtu.be/FkrYCXtiVI4>) in case your game has an update and/or DLC. If the base game is an `.xci`, no need to convert to `.nsp`. Just use `Update XCI` instead of `Update NSP`.\n5. Download the latest firmware from the firmware site. Do NOT extract this `.zip` file!\n6. In Ryubing, go to `Actions > Install Keys` and `Install Firmware`, then choose `Install from ZIP` for both and select your earlier `.zip` files.\n7. Click on `Options > Settings > User Interface`. In the \"Game Directories\" section, click on `Add`. Select the folder where you extracted (and/or merged) your game and select it. Click `Apply` then `OK`.\n8. Below the `File and Options` buttons, there should be options for the view mode of their UI. Click the icons toggle right below Options.\n9. Adjust the `Icon Size` slider as much as you want. Optionally, also toggle `Show Names` right next to the slider.\n10. Click on `Options > Settings > Input`. Configure controls as you like. Click `Apply` then `OK`.\n11. At the bottom left corner of the window, hit the refresh button.\n\\- From now on, whenever you open Ryubing quick access to your ROM will be located right in front of you. Just double click your game.\n\\- To make more ROMs show up in Ryubing, simply repeat steps 2 and 4 if needed. Extract to the same folder that you listed in step 7."
            },
            'ps1': { # duckstation
            ...
            'xbox': { # xemu
                "links": ["[Xemu](<https://xemu.app/docs/download/>)", "[ROMs](<https://vimm.net/vault/Xbox>)", "[BIOS](<https://github.com/K3V1991/Xbox-Emulator-Files/releases>)"],
